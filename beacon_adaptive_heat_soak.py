# Heat soak with thermal stability detection using Beacon proximity sensor data
#
# Copyright (C) 2025 Tom Glastonbury <t@tg73.net>
# Remixed 2025 Domingos Rodrigues <domingoslamas@gmail.com> for default klipper compatibily
#
...
class ThresholdPredictor:
	...
	def predict_threshold(self, maximum_z_change_microns, period_seconds):
		...
		Returns:
			The predicted heat soak threshold in nanometers per second.
		'''
		...
		parent_conn.close()
		if is_err:
			raise self.printer.command_error("Error predicting heat soak threshold: %s" % (result,))
		else:
			return result
	...
	def _load_training_data(self):
		...
		if not os.path.exists(path):
			raise FileNotFoundError(f"Beacon heat soak model training data file not found: {path}")
...
class BeaconAdaptiveHeatSoak:
	def __init__(self, config):
		...
		# The default layer quality for heat soak, which is used in conjunction with maximum_first_layer_duration
		...
		self.def_layer_quality = config.getint('default_layer_quality', 2, minval=1, maxval=5)
	...
	def _prepare_for_sampling_and_get_sampling_frequency(self):
		...
		# the beacon for sampling. This retry is a speculative workaround for that issue. The cause
	...
	def cmd_BEACON_WAIT_FOR_PRINTER_HEAT_SOAK(self, gcmd):
		if self.beacon is None:
			raise self.printer.command_error("Beacon is not available. Please ensure Printer is configured correctly.")
		...
		if threshold is None:
			...
			logging.info(f"{self.name}: predicted heat soak threshold for maximum Z change of {maximum_z_change_microns} microns (quality {layer_quality}) over {period} seconds: {threshold:.2f} nm/s")
		else:
			logging.info(f"{self.name}: using forced heat soak threshold: {threshold:.2f} nm/s")
					...
					if z_rate_ra.is_full():
						...
						if moving_average_trend_checks_passed is True or level_2_moving_average_trend_checks_passed is True:
							if elapsed < minimum_wait:
								...
								msg = f"Heat soak completed in {self._format_seconds(elapsed)}."
								gcmd.respond_info(msg)
								return

						if should_log:
							logging.info(
								f"{self.name}: elapsed={elapsed:.1f} s"
	...
	desc_BEACON_WAIT_FOR_PRINTER_HEAT_SOAK_CAPTURE_Z_RATES = "For developer use only. This command is used to run diagnostics for Beacon heat soak."
	...
	def cmd_TEST_PREDICT_ADAPTIVE_HEAT_SOAK_THRESHOLD(self, gcmd):
		...
		gcmd.respond_info(f"Predicted heat soak threshold for maximum Z change of {maximum_z_change_microns} microns over {period_seconds} seconds: {threshold:.2f} nm/s (prediction took {1000. * (end_time - start_time):.1f} ms)")

	desc_BEACON_WAIT_FOR_PRINTER_HEAT_SOAK_CAPTURE_BEACON_SAMPLES = "For developer use only. This command is used to run diagnostics for Beacon heat soak."
