...
TITLE = 'BEACON_AUTO_CALIBRATE Multi-point Probing'
...
class BeaconTrueZeroCorrection:
	...
	def reset_offset(self):
		self.gcode_move.base_position[2] = 0
		self.gcode_move.homing_position[2] = 0

	def _handle_homing_move_end(self, homing_state, rails):
		# Clear Z offset if Z axis is homed.
		if 2 in homing_state.get_axes():
			self.reset_offset()

	def _handle_motor_off(self):
		# Clear Z offset if motors are disabled.
		self.reset_offset()

	######
	# Commands
	######
	desc_BEACON_AUTO_CALIBRATE = "Automatically calibrates the Beacon probe. Extended with multi-point probing for improved true zero consistency. Use SKIP_MULTIPOINT_PROBING=1 to bypass."
	...
	def _validate_probing_region(self, range_x, range_y, span):
		...
		# TODO hardcoded min max probing zones
		probable_x = (20, 480)
		probable_y = (20, 460)
		...
		if not (
			...
			self.gcode.respond_raw(TITLE + 'error' + f'The required probing region ({span:.1f}x{span:.1f}) would probe outside the configured contact probing area.')
...
class ProbingSession:
	...
	def _finalize(self):
		if self._finalize_result == 'retry':
			...
			# TODO apply new offset
			self.tzc.gcode_move.base_position[2] = self._finalize_result
			self.tzc.gcode_move.homing_position[2] = self._finalize_result
