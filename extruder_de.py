...
class ExtruderStepper:
    ...
    def cmd_SET_PRESSURE_ADVANCE(self, gcmd):
        verbose = gcmd.get_int('VERBOSE',default= 0)
...
def add_printer_objects(config):
    ...
    for i in range(99):
        section = 'extruder_de'
        if i:
            section = 'extruder_de%d' % (i,)
        if not config.has_section(section):
            break
        pe = PrinterExtruder_DE(config.getsection(section), i)
