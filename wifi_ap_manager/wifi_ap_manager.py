...
try:
    ...
    print("Wifi restart sucesseful waiting 10s")
    time.sleep(10)
except:
...
if not disabled_ap:
    connected = False
    try:
        result = subprocess.run( 'sudo nmcli device wifi hotspot ifname wlan0 ssid DE-ALPHA password de-alpha', shell=True, capture_output=True, text=True )
        print(result.stdout)
        if result.returncode == 0:
            connected = True
    except:
